...
public class StaticAnimation extends DynamicAnimation implements AnimationProvider<StaticAnimation> {
    ...
    public StaticAnimation(ResourceLocation baseAnimPath, float convertTime, boolean repeatPlay, String registryName,
            Armature armature, boolean multilayer) {
        super(convertTime, repeatPlay);
    ...
    public static void loadAllJointsClip(IResourceManager resourceManager, StaticAnimation animation) throws Exception {
        JsonModelLoader modelLoader = (new JsonModelLoader(resourceManager, animation.resourceLocation));
    ...
    public void setLinkAnimation(final DynamicAnimation fromAnimation, Pose startPose, boolean isOnSameLayer,
            float convertTimeModifier, LivingEntityPatch<?> entitypatch, LinkAnimation dest) {
        ...
        AnimationProperty.PlaybackSpeedModifier playSpeedModifier = this.getRealAnimation()
                .getProperty(StaticAnimationProperty.PLAY_SPEED_MODIFIER).orElse(null);
        ...
        Map<String, JointTransform> data2 = this.getPoseByTime(entitypatch, nextStartTime, 0.0F)
                .getJointTransformData();
        ...
        if (entitypatch.isLogicalClient()) {
            ...
            if (entry != null) {
                joint1.removeIf(
                        (jointName) -> entry
                                .isMasked(
                                        fromAnimation.getProperty(ClientAnimationProperties.LAYER_TYPE)
                                                .orElse(Layer.LayerType.BASE_LAYER) == Layer.LayerType.BASE_LAYER
            ...
            if (entry2 != null) {
                joint2.removeIf((jointName) -> entry2.isMasked(this.getProperty(ClientAnimationProperties.LAYER_TYPE)
                        .orElse(Layer.LayerType.BASE_LAYER) == Layer.LayerType.BASE_LAYER
    ...
    protected TypeFlexibleHashMap<EntityState.StateFactor<?>> getStatesMap(LivingEntityPatch<?> entitypatch,
            DynamicAnimation animation, float time) {
        return this.stateSpectrum.getStateMap(entitypatch, time);
    }

    protected <T> T getState(EntityState.StateFactor<T> stateFactor, LivingEntityPatch<?> entitypatch,
            DynamicAnimation animation, float time) {
    ...
    @Override
    public final TypeFlexibleHashMap<EntityState.StateFactor<?>> getStatesMap(LivingEntityPatch<?> entitypatch,
            float time) {
    ...
    public StaticAnimation newConditionalTimePair(Function<LivingEntityPatch<?>, Integer> condition, float start,
            float end) {
