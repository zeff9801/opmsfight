...
public class GrapplingTryAnimation extends AttackAnimation {
    ...
    public GrapplingTryAnimation(float convertTime, float antic, float preDelay, float contact, float recovery, Collider collider, Joint colliderJoint, String path, AnimationProvider<?> grapplingHitAnimation, AnimationProvider<?> interactAnimation, AnimationProvider<?> failAnimation, Armature armature) {
        this(convertTime, antic, preDelay, contact, recovery, Hand.MAIN_HAND, collider, colliderJoint, path, grapplingHitAnimation, interactAnimation, failAnimation, armature);
    }

    public GrapplingTryAnimation(float convertTime, float antic, float preDelay, float contact, float recovery, Hand hand, Collider collider, Joint colliderJoint, String path, AnimationProvider<?> grapplingHitAnimation, AnimationProvider<?> interactAnimation, AnimationProvider<?> failAnimation, Armature armature) {
            ...
            if (hitEntity != null) {
                ...
                if (list.contains(hitEntity)) {
                    ...
                    if (entitypatch.tryHurt(dmgSource, 0.0F).resultType.dealtDamage()) {
                        ...
                        if (hitEntityPatch != null) {
                            hitEntity.lookAt(EntityAnchorArgument.Type.FEET, entitypatch.getOriginal().position());
