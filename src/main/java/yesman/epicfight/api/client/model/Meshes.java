...
public class Meshes implements IFutureReloadListener {
    ...
    public static <M extends RawMesh> M getOrCreateRawMesh(IResourceManager rm, ResourceLocation rl, MeshContructor<RawMesh.RawModelPart, VertexBuilder, M> constructor) {
    ...
    @SuppressWarnings("unchecked")
    public static <M extends AnimatedMesh> M getOrCreateAnimatedMesh(IResourceManager rm, ResourceLocation rl, MeshContructor<AnimatedMesh.AnimatedModelPart, AnimatedVertexBuilder, M> constructor) {
        return (M) MESHES.computeIfAbsent(rl, (key) -> {
            JsonModelLoader jsonModelLoader = new JsonModelLoader(rm, wrapLocation(rl));
