...
import yesman.epicfight.api.utils.ModelRendererUtil;
...
public class VanillaModelTransformer extends HumanoidModelTransformer {
	public static final PartTransformer<ModelRenderer.ModelBox> HEAD = new SimpleTransformer(9);
	public static final PartTransformer<ModelRenderer.ModelBox> LEFT_FEET = new SimpleTransformer(5);
	public static final PartTransformer<ModelRenderer.ModelBox> RIGHT_FEET = new SimpleTransformer(2);
	public static final PartTransformer<ModelRenderer.ModelBox> LEFT_ARM = new LimbPartTransformer(16, 17, 19, 19.0F, false, AABBUtil.ofSize(new Vector3d(-6.0D, 18.0D, 0), 8.0D, 14.0D, 8.0D));
	public static final PartTransformer<ModelRenderer.ModelBox> RIGHT_ARM = new LimbPartTransformer(11, 12, 14, 19.0F, false, AABBUtil.ofSize(new Vector3d(6.0D, 18.0D, 0), 8.0D, 14.0D, 8.0D));
	public static final PartTransformer<ModelRenderer.ModelBox> LEFT_LEG = new LimbPartTransformer(4, 5, 6, 6.0F, true, AABBUtil.ofSize(new Vector3d(-2.0D, 6.0D, 0), 8.0D, 14.0D, 8.0D));
	public static final PartTransformer<ModelRenderer.ModelBox> RIGHT_LEG = new LimbPartTransformer(1, 2, 3, 6.0F, true, AABBUtil.ofSize(new Vector3d(2.0D, 6.0D, 0), 8.0D, 14.0D, 8.0D));
	public static final PartTransformer<ModelRenderer.ModelBox> CHEST = new ChestPartTransformer(8, 7, 18.0F, AABBUtil.ofSize(new Vector3d(0, 18.0D, 0), 12.0D, 14.0D, 6.0D));
	
	@OnlyIn(Dist.CLIENT)
	static record VanillaModelPartition(PartTransformer<ModelRenderer.ModelBox> partTransformer, ModelRenderer modelPart, String partName) {
	}
	
	@Override
	public AnimatedMesh transformArmorModel(ResourceLocation modelLocation, BipedModel<?> humanoidModel) {
		List<VanillaModelPartition> boxes = Lists.newArrayList();

		//Remove entity animation
		ModelRendererUtil.resertPartAnimation(humanoidModel.head);
		ModelRendererUtil.resertPartAnimation(humanoidModel.hat);
		ModelRendererUtil.resertPartAnimation(humanoidModel.body);
		ModelRendererUtil.resertPartAnimation(humanoidModel.leftArm);
		ModelRendererUtil.resertPartAnimation(humanoidModel.rightArm);
		ModelRendererUtil.resertPartAnimation(humanoidModel.leftLeg);
		ModelRendererUtil.resertPartAnimation(humanoidModel.rightLeg);
	...
	private static AnimatedMesh bakeMeshFromCubes(List<VanillaModelPartition> partitions) {
		...
		poseStack.mulPose(QuaternionUtils.XP.rotationDegrees(180.0F).toVanillaQuaternion());
		poseStack.translate(0.0F, -24.0F, 0.0F);
		
		for (VanillaModelPartition modelpartition : partitions) {
			bake(poseStack, modelpartition.partName, Lists.newArrayList(), modelpartition, modelpartition.modelPart, vertices, indices, indexCounter, false);
		}
		
		return SingleGroupVertexBuilder.loadVertexInformation(vertices, indices);
	}
	
	private static void bake(MatrixStack poseStack, String baseName, List<Integer> pathIndices, VanillaModelPartition modelpartition, ModelRenderer part, List<SingleGroupVertexBuilder> vertices, Map<MeshPartDefinition, IntList> indices, PartTransformer.IndexCounter indexCounter, boolean bindPart) {
		poseStack.pushPose();
		poseStack.translate(part.x, part.y, part.z);
		poseStack.mulPose(new Quaternionf().rotationZYX(part.zRot, part.yRot, part.xRot).toVanillaQuaternion());

		// Generate partName from baseName and pathIndices
		String partName = baseName;
		if (!pathIndices.isEmpty()) {
			partName += "_" + pathIndices.stream().map(String::valueOf).collect(Collectors.joining("_"));
		}

		List<Integer> newPathIndices = new ArrayList<>(pathIndices);

		
//		if (!bindPart) { //We do not have access to scale parts of the model
//			poseStack.scale(part.xScale, part.yScale, part.zScale);
//		}
		
//		if (bindPart) {
//			newPathIndices.add(partName);
//		}
		...
		if (part.visible) {
			...
			if (bindPart) {
				...
				partDefinition = VanillaMeshPartDefinition.of(partName, newPathIndices, invertedParentTransform, modelpartition.modelPart);
			}
		...
		// Iterate over children using indices since children is a List in 1.16.5
		for (int i = 0; i < part.children.size(); i++) {
			ModelRenderer child = part.children.get(i);
			List<Integer> childPathIndices = new ArrayList<>(pathIndices);
			childPathIndices.add(i);
			bake(poseStack, baseName, childPathIndices, modelpartition, child, vertices, indices, indexCounter, true);
		}
...
//		}
	...
	@OnlyIn(Dist.CLIENT)
	static class SimpleTransformer extends PartTransformer<ModelRenderer.ModelBox> {
		final int jointId;
		...
		public void bakeCube(MatrixStack poseStack, MeshPartDefinition partDefinition, ModelRenderer.ModelBox cube, List<SingleGroupVertexBuilder> vertices, Map<MeshPartDefinition, IntList> indices, PartTransformer.IndexCounter indexCounter) {
			for (ModelRenderer.TexturedQuad quad : cube.polygons) {
				Vector3f norm = quad.normal;
	...
	@OnlyIn(Dist.CLIENT)
	static class ChestPartTransformer extends PartTransformer<ModelRenderer.ModelBox> {
		...
		final AxisAlignedBB noneAttachmentArea;
		final float yClipCoord;

		public ChestPartTransformer(int upperJoint, int lowerJoint, float yBasis, AxisAlignedBB noneAttachmentArea) {
		...
		@Override
		public void bakeCube(MatrixStack poseStack, MeshPartDefinition partDefinition, ModelRenderer.ModelBox cube, List<SingleGroupVertexBuilder> vertices, Map<MeshPartDefinition, IntList> indices, PartTransformer.IndexCounter indexCounter) {
			Vector3d centerOfCube = getCenterOfCube(poseStack, cube);
			...
			for (AnimatedPolygon polygon : xClipPolygons) {
				...
				if (vertexWeights.size() > 0) {
					for (VertexWeight vertexWeight : vertexWeights) {
						...
						ModelRenderer.PositionTextureVertex pos4 = new ModelRenderer.PositionTextureVertex(clipPos2, pos0.u, textureV);
						ModelRenderer.PositionTextureVertex pos5 = new ModelRenderer.PositionTextureVertex(clipPos1, pos1.u, textureV);
	...
	@OnlyIn(Dist.CLIENT)
	static class LimbPartTransformer extends PartTransformer<ModelRenderer.ModelBox> {
		...
		public LimbPartTransformer(int upperJoint, int lowerJoint, int middleJoint, float yClipCoord, boolean bendInFront, AxisAlignedBB noneAttachmentArea) {
		...
		public void bakeCube(MatrixStack poseStack, MeshPartDefinition partDefinition,  ModelRenderer.ModelBox cube, List<SingleGroupVertexBuilder> vertices, Map<MeshPartDefinition, IntList> indices, PartTransformer.IndexCounter indexCounter) {
			...
			for (AnimatedPolygon quad : polygons) {
				Vector3f norm = quad.normal;
				norm.mul(poseStack.last().normal().adjugateAndDet());
	...
	static Vector3d getCenterOfCube(MatrixStack poseStack, ModelRenderer.ModelBox cube) {
		...
		for (ModelRenderer.TexturedQuad quad : cube.polygons) {
			for (ModelRenderer.PositionTextureVertex v : quad.vertices) {
	...
	static Vector3f getClipPoint(Vector3f pos1, Vector3f pos2, float yClip) {
		Vector3f direct = pos2;
		direct.sub(pos1);
		direct.mul((yClip - pos1.y()) / (pos2.y() - pos1.y()));
		
		Vector3f clipPoint = pos1;
	...
	@OnlyIn(Dist.CLIENT)
	public record VanillaMeshPartDefinition(String partName, List<Integer> pathIndices, OpenMatrix4f invertedParentTransform, ModelRenderer root) implements MeshPartDefinition {
		public static MeshPartDefinition of(String partName) {
			return new VanillaMeshPartDefinition(partName, null, null, null);
		}
		
		public static MeshPartDefinition of(String partName, List<Integer> pathIndices, OpenMatrix4f invertedParentTransform, ModelRenderer root) {
			return new VanillaMeshPartDefinition(partName, pathIndices, invertedParentTransform, root);
		}
		...
		public Supplier<OpenMatrix4f> getModelPartAnimationProvider() {
			return this.root == null ? () -> null : () -> {
				...
				ModelRenderer part = this.root;
				int idx = 0;

				for (int i = 0; i < this.pathIndices.size(); i++) {
					int childIndex = this.pathIndices.get(i);
					part = part.children.get(childIndex);
					boolean isLast = (i == this.pathIndices.size() - 1);
					this.progress(part, poseStack, isLast);
				}
				...
				ModelRenderer lastPart = part; //
				OpenMatrix4f partAnimation = OpenMatrix4f.mulMatrices(animParentTransform,
																	  new OpenMatrix4f().mulBack(OpenMatrix4f.fromQuaternion(new Quaternionf().rotationZYX(part.zRot, part.yRot, part.xRot)).transpose().invert())
																						.translate(new Vec3f(lastPart.x - part.x, lastPart.y - part.y, lastPart.z - part.z).scale(0.0625F))
																						.mulBack(OpenMatrix4f.fromQuaternion(new Quaternionf().rotationZYX(part.zRot, part.yRot, part.xRot)).transpose())
																						.mulBack(OpenMatrix4f.fromQuaternion(new Quaternionf().rotationZYX(lastPart.zRot - part.zRot, lastPart.yRot - part.yRot, lastPart.xRot - part.xRot)).transpose())
		...
		private void progress(ModelRenderer part, MatrixStack poseStack, boolean last) {
