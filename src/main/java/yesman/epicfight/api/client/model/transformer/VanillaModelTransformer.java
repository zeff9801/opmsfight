...
public class VanillaModelTransformer extends HumanoidModelTransformer {
	...
	public static final PartTransformer<ModelPart.Cube> LEFT_ARM = new LimbPartTransformer(16, 17, 19, 19.0F, false, AxisAlignedBB.ofSize(new Vector3d(-6.0D, 18.0D, 0), 8.0D, 14.0D, 8.0D));
	public static final PartTransformer<ModelPart.Cube> RIGHT_ARM = new LimbPartTransformer(11, 12, 14, 19.0F, false, AxisAlignedBB.ofSize(new Vector3d(6.0D, 18.0D, 0), 8.0D, 14.0D, 8.0D));
	public static final PartTransformer<ModelPart.Cube> LEFT_LEG = new LimbPartTransformer(4, 5, 6, 6.0F, true, AxisAlignedBB.ofSize(new Vector3d(-2.0D, 6.0D, 0), 8.0D, 14.0D, 8.0D));
	public static final PartTransformer<ModelPart.Cube> RIGHT_LEG = new LimbPartTransformer(1, 2, 3, 6.0F, true, AxisAlignedBB.ofSize(new Vector3d(2.0D, 6.0D, 0), 8.0D, 14.0D, 8.0D));
	public static final PartTransformer<ModelPart.Cube> CHEST = new ChestPartTransformer(8, 7, 18.0F, AxisAlignedBB.ofSize(new Vector3d(0, 18.0D, 0), 12.0D, 14.0D, 6.0D));
	...
	public AnimatedMesh transformArmorModel(ResourceLocation modelLocation, BipedModel<?> humanoidModel) {
		...
		humanoidModel.head.setPos(humanoidModel.head.getInitialPose());
		humanoidModel.hat.setPos(humanoidModel.hat.getInitialPose());
		humanoidModel.body.setPos(humanoidModel.body.getInitialPose());
		humanoidModel.leftArm.setPos(humanoidModel.leftArm.getInitialPose());
		humanoidModel.rightArm.setPos(humanoidModel.rightArm.getInitialPose());
		humanoidModel.leftLeg.setPos(humanoidModel.leftLeg.getInitialPose());
		humanoidModel.rightLeg.setPos(humanoidModel.rightLeg.getInitialPose());
	...
	private static void bake(MatrixStack poseStack, String partName, VanillaModelPartition modelpartition, ModelPart part, List<SingleGroupVertexBuilder> vertices, Map<MeshPartDefinition, IntList> indices, List<String> path, PartTransformer.IndexCounter indexCounter, boolean bindPart) {
		MatrixStack initialPose = part.getInitialPose();
	...
	static class SimpleTransformer extends PartTransformer<ModelPart.Cube> {
		...
		public void bakeCube(MatrixStack poseStack, MeshPartDefinition partDefinition, ModelPart.Cube cube, List<SingleGroupVertexBuilder> vertices, Map<MeshPartDefinition, IntList> indices, PartTransformer.IndexCounter indexCounter) {
	...
	static Vector3d getCenterOfCube(MatrixStack poseStack, ModelPart.Cube cube) {
	...
	public record VanillaMeshPartDefinition(String partName, List<String> path, OpenMatrix4f invertedParentTransform, ModelPart root) implements MeshPartDefinition {
		...
		private void progress(ModelPart part, MatrixStack poseStack, boolean last) {
