...
public class CubicBezierCurve {
	...
	/**
	 * Get more informations about cubic Bézier curve here:
	...
	public static List<Vector3d> getBezierInterpolatedPoints(List<Vector3d> points, int interpolatedResults) {
	 ...
	 * @return
	 */
	public static List<Vector3d> getBezierInterpolatedPoints(List<Vector3d> points, int sliceBegin, int sliceEnd, int interpolatedResults) {
		if (points.size() < 3) {
			return null;
		}
		...
		int size = points.size();
		List<Vector3d> interpolatedPoints = Lists.newArrayList();
		...
		for (int i = sliceBegin; i < sliceEnd; i++) {
			...
			for (int j = 0; j < interpolatedResults + 1; j++) {
				...
				interpolatedPoints.add(new Vector3d(cubicBezier(start.x, end.x, x_av, x_bv, t)
