...
public class RenderEngine {
	...
	public void setCameraRotation(float x, float y) {
		...
		this.cameraXRotO += f;
		this.cameraYRotO += f1;
		this.cameraXRotO = MathHelper.clamp(this.cameraXRotO, -90.0F, 90.0F);
	...
	public void correctCamera(CameraSetup event, float partialTicks) {
		if (this.isPlayerRotationLocked) {
			...
			if (!cameraType.isFirstPerson()) {
				...
				camera.setPosition(MathHelper.lerp((double)partialTicks, cameraEntity.xo, cameraEntity.getX()),
						MathHelper.lerp((double)partialTicks, cameraEntity.yo, cameraEntity.getY()) + MathHelper.lerp(partialTicks, camera.eyeHeightOld, camera.eyeHeight),
						MathHelper.lerp((double)partialTicks, cameraEntity.zo, cameraEntity.getZ()));
