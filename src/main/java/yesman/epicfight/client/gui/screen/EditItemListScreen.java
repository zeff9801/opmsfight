...
public class EditItemListScreen extends Screen {
	...
	protected void init() {
		...
		this.children.add(new Button(this.width / 2 + 125, this.height - 26, 60, 20, DialogTexts.GUI_DONE, (button) -> {
	...
	public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
		...
		drawString(matrixStack, this.font, new StringTextComponent("Item List").withStyle(TextFormatting.UNDERLINE), 28, 10, 16777215);
		drawString(matrixStack, this.font, new StringTextComponent("Seleted Items").withStyle(TextFormatting.UNDERLINE), 28, this.height-114, 16777215);
		...
		public void addItem(Item item) {
			...
			if (EditItemListScreen.this.opponentRegistered.contains(item)) {
				onTooltip = (button, matrixStack, mouseX, mouseY) -> {
					ITextComponent displayName = ((ItemButton)button).item.getName(ItemStack.EMPTY);
