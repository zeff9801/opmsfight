...
public class EnderDragonPatch extends MobPatch<EnderDragonEntity> {
	public static final EntityPredicate DRAGON_TARGETING = (new EntityPredicate()).range(64.0D).ignoreInvisibilityTesting();
	...
	public void onJoinWorld(EnderDragonEntity entityIn, EntityJoinWorldEvent event) {
		...
		IPhase currentPhase = this.original.phaseManager.getCurrentPhase();
		PhaseType<?> startPhase = (currentPhase == null || !(currentPhase instanceof PatchedDragonPhase)) ? PatchedPhases.FLYING : this.original.phaseManager.getCurrentPhase().getPhase();
	...
	public void updateMotion(boolean considerInaction) {
		if (this.original.getHealth() <= 0.0F) {
			...
			IPhase phase = this.original.getPhaseManager().getCurrentPhase();
	...
	public void serverTick(LivingUpdateEvent event) {
		...
		AxisAlignedBB bodyBoundingBox = bodyPart.getBoundingBox();
		List<Entity> list = this.original.level.getEntities(this.original, bodyBoundingBox);
	...
	public void setStunShield(float value) {
		...
		if (value <= 0) {
			IPhase currentPhase = this.original.getPhaseManager().getCurrentPhase();
	...
	public void rotateTo(Entity target, float limit, boolean partialSync) {
        ...
        float degree = 180.0F - (float)Math.toDegrees(MathHelper.atan2(d0, d1));
	...
	public void updateTipPoints() {
		...
		if (!this.tipPointAnimations.isEmpty()) {
			...
			this.xRoot += MathHelper.clamp(((float)Math.toDegrees(Math.atan2(zdiff, zdistance)) - this.xRoot), -1.0F, 1.0F);
			this.zRoot += MathHelper.clamp(((float)Math.toDegrees(Math.atan2(xdiff, xdistance)) - this.zRoot), -1.0F, 1.0F);
			float averageY = (yFrontL + yFrontR + yBackL + yBackR) * 0.25F;
			
			if (!this.isLogicalClient()) {
				float dy = averageY - entityPosY;
				this.original.move(MoverType.SELF, new Vector3d(0.0F, dy, 0.0F));
			}
		}
	}
	...
	public OpenMatrix4f getModelMatrix(float partialTicks) {
		return MathUtils.getModelMatrixIntegral(0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, this.original.yRotO, this.original.yRot, partialTicks, -1.0F, 1.0F, -1.0F);
	}
	
	@Override
	public double getAngleTo(Entity entityIn) {
		Vector3d a = this.original.getLookAngle().scale(-1.0D);
