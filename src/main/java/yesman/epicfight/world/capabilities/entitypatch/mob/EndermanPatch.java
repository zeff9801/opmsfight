...
public class EndermanPatch extends MobPatch<EndermanEntity> {
	...
	public void onJoinWorld(EndermanEntity enderman, EntityJoinWorldEvent event) {
		if (enderman.level.dimension() == World.END) {
			if (MathUtils.horizontalDistanceSqr(enderman.position()) < 40000.0D) {
	...
	protected void toRaging() {
		...
		if (!this.original.isNoAi()) {
			...
			this.original.goalSelector.addGoal(1, this.rageAttacks);
			this.original.targetSelector.addGoal(3, this.rageTargeting);
			this.original.getEntityData().set(EndermanEntity.DATA_CREEPY, Boolean.valueOf(true));
			this.original.addEffect(new EffectInstance(EpicFightMobEffects.STUN_IMMUNITY.get(), 120000));
	...
	protected void toNormal() {
		...
		if (!this.original.isNoAi()) {
			...
			if (this.original.getTarget() == null) {
				this.original.getEntityData().set(EndermanEntity.DATA_CREEPY, Boolean.valueOf(false));
			}
		...
		public void tick() {
			...
			if (this.delayCounter-- < 0 && !this.mobpatch.getEntityState().inaction()) {
				...
				if (flag) {
		        	...
		        	this.mob.level.playSound((PlayerEntity)null, this.mob.xo, this.mob.yo, this.mob.zo, SoundEvents.ENDERMAN_TELEPORT, this.mob.getSoundSource(), 1.0F, 1.0F);
